...
\begin{frame}
    \titlepage
\end{frame}
\slideq{Espace connexe}{1/5}
\slider{$X$ est connexe si pour tout couple $\l U,V\r$ d'ouverts tels que $X=U\sqcup V$, $U=\varnothing$ ou $V=\varnothing$\linebreak De manière équivalente, les seuls ouverts-fermés de $X$ sont $X$ et $\varnothing$}{1/5}
\slideq{Espace totalement discontinu}{2/5}
\slider{$X$ est totalement discontinu si les composantes connexes de $X$ sont les singletons}{2/5}
\slideq{Espace topologique séparé}{3/5}
\slider{$X$ est un espace toplogique séparé si pour tout $x\neq y$, il existe $V_x$ et $V_y$ des voisinages ouverts de $x$ et $y$ tels que $V_x\cap V_y=\varnothing$\linebreak De manière équivalente, $\left\{\l x,x\r,x\in X\right\}$ est fermé dans $X^2$}{3/5}
