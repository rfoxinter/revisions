...
async function fill_svg(file) {
    ...
    if (res == 200) {
        ...
        if (document.readyState !== "loading") {
            ...
            document.getElementById('flashcard-front').src = 'data:text/html;base64,' + btoa(start + unescape(encodeURIComponent("<p style='color: #606c71;'>" + 'Impossible de charger le fichier</br>Regarder le terminal (F12 puis &OpenCurlyQuote;Console&CloseCurlyQuote;) pour plus d\'informations' + "</p>")) + end);
        } else {
            document.addEventListener("DOMContentLoaded", (event) => {
                document.getElementById('up-button').style.display = 'none';
                document.getElementById('title').innerHTML = 'Erreur';
        ...
        if (document.readyState !== "loading") {
            ...
            document.getElementById('flashcard-front').src = 'data:text/html;base64,' + btoa(start + unescape(encodeURIComponent("<p style='color: #606c71;'>" + 'Fichier introuvable' + "</p>")) + end);
        } else {
            document.addEventListener("DOMContentLoaded", (event) => {
                document.getElementById('up-button').style.display = 'none';
                document.getElementById('title').innerHTML = 'Erreur ' + res;
...
function new_card(corr) {
    ...
    if (nth + 1 == ques.length) {
        ...
        nth += 1;
        document.getElementById('flashcard-front').src = 'data:text/html;base64,' + btoa(start + unescape(encodeURIComponent("<p style='color: #606c71;'>" + 'Terminé' + "</p>")) + end);
        document.getElementById('flip').disabled = true;
...
function _close() {
    ...
    document.getElementById('flashcard-front').src = 'data:text/html;base64,' + btoa(start + unescape(encodeURIComponent("<p style='color: #606c71;'>" + '' + "</p>")) + end);
    document.getElementById('incor').disabled = true;
    document.getElementById('corr').disabled = true;
    document.getElementById('flip').disabled = true;
