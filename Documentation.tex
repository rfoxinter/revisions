...
Pour utiliser des commandes avec des parenthèses automatiques (comme pour $\oldsup$), il est possible de faire\footnote{Cette commande est déjà définie dans \hyperlink{section.20}{\texttt{Usuelles.sty}}} :
...
\footnotetext{Ne pas confondre cette commande avec \mintinline{latexmath}?\mat? de \hyperlink{section.11}{\texttt{Matrices.sty}}}
...
\toggleanalyselimits\subsection{La commande {\color{keycolour}\texttt{\textbackslash autoanalyselimits}}}
Il est possible de remettre le comportement automatiques de \LaTeX{} pour l'affichage des indices avec \mintinline{latex}?\autoanalyselimits?. La commande \mintinline{latex}?\toggleanalyselimits? remettra alors le comportement par défaut du package (donc avec \mintinline{latexmath}?\limits?).
...
Le package \hyperlink{section.17}{\texttt{Tools.sty}} sera importé automatiquement avec \texttt{Matrices.sty}.
