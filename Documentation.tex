...
\usepackage{dsfont}
\makeatletter
\DeclareMathAlphabet{\mathdds}{U}{\ds@whichfont}{m}{n}
\makeatother
\usepackage[amsbb]{output/htmlpreambule}
\usepackage{dsft,bbmt}
\usepackage{output/tools}
\usepackage[bigopsymb]{output/bigoperators}
\usepackage{output/matrices,output/al,output/analyse,output/analysecomplexe,output/arithmetique,output/arrows,output/complexes,output/equivalents,output/galois,output/geometrie,output/hats,output/polynomes,output/probas,output/sffont,output/structures,output/trigo,output/tables,output/topologie,output/usuelles}
\makeatletter
...
Pour utiliser des commandes avec des parenthèses automatiques (comme pour $\oldsup$), il est possible de placer dans le préambule\footnote{Cette commande est déjà définie dans \hyperlink{section.25}{\texttt{Usuelles.sty}}}:
...
\footnotetext{Il est par exemple possible de produire $\fakebold{\amsbb{R}}$ avec \mintinline{latex}?$\fakebold{\amsbb{R}}$?\\Il est recommandé de n'utiliser cette commande que lorsque les caractères en gras ne sont pas disponibles avec \mintinline{latex}?\boldmath?\\Exemples de comparaison \mintinline{latex}?\boldmath? - \mintinline{latexmath}?\fakebold? avec le caractère normal: {\boldmath$\mathcal M$}$\fakebold{\mathcal M}$$\mathcal M$; {\boldmath$\mathbb R$}$\fakebold{\mathbb R}$$\mathbb R$}
\mmcmd[\footnotemark]{\mathbb{CQRhkl1}}{$\mathbbm{CQRhkl1}$}
\footnotetext{Correspond à une version en type 1 de la police \texttt{bbm}\\La version de l'\amslg{} peut être rétablie avec \mintinline{latex}?\usepackage[amsbb]{premabule}?}
\mcmd[\footnotemark]{\amsbb{CQR}\Bbbk}
\footnotetext{Correspond à la version de l'\amslg{} de la commande \mintinline{latexmath}?\mathbb?\\Cette commande n'est plus disponible avec \mintinline{latex}?\usepackage[amsbb]{premabule}?}
\mcmd[\footnotemark]{\varphi}
...
\pagebreak
\hrule
\mcmd[\footnotemark]{\varepsilon}
\footnotetext{Correspond à la commande usuelle \mintinline{latexmath}?\epsilon?}
\mcmd[\footnotemark]{\epsilon}
\footnotetext{Correspond à la commande usuelle \mintinline{latexmath}?\varepsilon?}
\mcmd[\footnotemark]{\subset}
\footnotetext{Correspond à la commande usuelle \mintinline{latexmath}?\subseteq?}
\mcmd{\subopn}
\mcmd{\subcls}
\mcmd[\footnotemark]{\bbA\bbz}
\footnotetext{Raccourci pour la commande usuelle \mintinline{latexmath}?\mathbb?}
\mcmd[\footnotemark]{\calA\calZ}
\footnotetext{Raccourci pour la commande usuelle \mintinline{latexmath}?\mathcal?}
\mcmd[\footnotemark]{\frakA\frakz}
\footnotetext{Raccourci pour la commande usuelle \mintinline{latexmath}?\mathfrak?}
...
\footnotetext{Ne pas confondre cette commande avec \mintinline{latexmath}?\mat? de \hyperlink{section.16}{\texttt{Matrices.sty}}}
...
Le package \hyperlink{section.22}{\texttt{Tools.sty}} sera importé automatiquement avec \texttt{Analyse.sty}.
...
\mcmd{\fdiv{f}}
\mcmd{\oldpic}
\mcmd{\pic{\mathbb{C}/\Lambda}}
\mcmd{\pic[0]{\mathbb{C}/\Lambda}}
...
Le package \hyperlink{section.22}{\texttt{Tools.sty}} sera importé automatiquement avec \texttt{BigOperators.sty}.
...
\mcmd[\footnotemark]{\oldcoprod}
\footnotetext{Correspond à la commande usuelle \mintinline{latexmath}?\coprod?}
\mcmd{\coprod{\alpha\in A}{}{V_\alpha}}
...
\footnotetext{Cette commande ne fonctionne qu'avec la police de l'\amslg}
\mcmd{\jj}
\mcmd[\footnotemark]{\jjj}
\footnotetext{Cette commande ne fonctionne qu'avec la police de l'\amslg}
\mcmd{\oldIm}
\mcmd{\Im{z}}
\mcmd[\footnotemark]{\altoldIm}
\footnotetext{Correspond à la commande usuelle \mintinline{latexmath}?\Im?}
\mcmd{\altIm{z}}
\mcmd{\oldRe}
\mcmd{\Re{z}}
\mcmd[\footnotemark]{\altoldRe}
\footnotetext{Correspond à la commande usuelle \mintinline{latexmath}?\Re?}
\mcmd{\altRe{z}}
\hsection{Dsft.sty}

\textcolor{red}{Ce package n'est plus disponible dans ce dépôt.}
...
\textcolor{red}{Il est recommandé d'utiliser \mintinline{latexmath}?\mathbb{1}? pour obtenir $\mathbb 1$.}
...
\mhd
\mcmd{\oldgal}
\mcmd{\gal{\mathbb{L}}{\mathbb{K}}}
\mcmd{\dec_{\mathbb{K}}(P)}
\mcmd{\edeg{\mathbb{L}}{\mathbb{K}}}
\mcmd{\sdeg{\mathbb{L}}{\mathbb{K}}}
\mcmd{\ideg{\mathbb{L}}{\mathbb{K}}}
\mcmd{\pideg{\mathbb{L}}{\mathbb{K}}}
\mcmd{\olddegsep}
\mcmd{\degsep[\mathbb{K}]{\alpha}}
\mcmd{\olddegins}
\mcmd{\degins[\mathbb{K}]{\alpha}}
\mcmd{\olddegpi}
\mcmd{\degpi[\mathbb{K}]{\alpha}}
\hsection{Geometrie.sty}
Le package \hyperlink{section.8}{\texttt{TBigoperators.sty}} sera importé automatiquement avec \texttt{Geometrie.sty}.
\vspace{0.5cm}
\mhd
\mcmd{\tg}
\mcmd{\oldder}
...
\mcmd{\hhat{ABCDEFFGH}}
\mcmd{\ccheck{AB}}
\mcmd{\ccheck{ABCDEFFGH}}
\hsection{Matrices.sty}
Le package \hyperlink{section.22}{\texttt{Tools.sty}} sera importé automatiquement avec \texttt{Matrices.sty}.
...
Le package \hyperlink{section.22}{\texttt{Tools.sty}} sera importé automatiquement avec \texttt{Usuelles.sty}.
