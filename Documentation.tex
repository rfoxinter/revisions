...
\phantomsection\label{ldef}\begin{Verbatim}[vspace=0pt]
\mathcode`l="8000
\begingroup
\makeatletter
\lccode`\~=`\l
...
Pour utiliser des commandes avec des parenthèses automatiques (comme pour $\oldsup$), il est possible de faire\footnote{Cette commande est déjà définie dans \hyperlink{section.17}{\Verb?Usuelles.sty?}} :
...
La commande \Verb?$\sup{\left\{x\in\mathbb{Q}\;\sq\;x^2<2\right\}}$? donne donc\linebreak$\sup{\left\{x\in\mathbb{Q}\;\sq\;x^2<2\right\}}$.
...
Il est aussi possible de télécharger la version la plus récente de ce fichier en cliquant sur le titre.
...
\mcmd[\footnotemark]{l}
\footnotetext{Correspond à la commande usuelle \Verb?\ell?\\Le $l$ peut être redéfini en $\oldl$ avec le code donné \hyperref[ldef]{ici}}
...
\footnotetext{Le \Verb?\oplus? utilisé est celui de \href{https://ctan.org/pkg/stmaryrd}{\color{black}\Verb?stmaryrd?}\\Pour récupérer celui de \LaTeX, il est possible d'utiliser la commande \Verb?\let\oldoplus\oplus? avant \Verb?\usepackage{al}? puis de faire \Verb?\let\oplus\oldoplus? après importation\\Comparaison \LaTeX~- \href{https://ctan.org/pkg/stmaryrd}{\color{black}\Verb?stmaryrd?} avec le plus normal $\oldoplus\oplus+$}
\mcmd[\footnotemark]{\matgl{n}{\mathbb{K}}}
\footnotetext{Le \Verb?\matgl? de \Verb?AL.sty? correspond à la commande \Verb?\gl? de \hyperlink{section.10}{\Verb?Matrices.sty?} qui a été renommé}
\mcmd{\gl{E}}
...
\footnotetext{Ne pas confondre cette commande avec \Verb?\mat? de \hyperlink{section.10}{\Verb?Matrices.sty?}}
...
Il est aussi possible de faire des arcs en important le package \texttt{Complexes.sty} avec l'option \texttt{arc}, en utilisant \Verb?\usepackage[arc]{complexes}?.
...
\mcmd{\arc{ABCDEFFGH}}
\hsection{Dsft.sty}
Ce package remplace le $\mathds 1$ du package \Verb?Dsfonts.sty? disponible sur \href{https://www.ctan.org/pkg/doublestroke}{CTAN} et introduit quelques symboles.
...
Pour utiliser ce package, il faut copier les fichiers \Verb?dsrom12.pfb? et \Verb?dsrom12.tfm? dans les dossiers où ils sont actuellement avec \Verb?dsfonts? (et éventuellement créer une copie des anciens fichiers).
...
\footnotetext{Les caractères \Verb?\&? sont utilisés au lieu du \Verb?&? utilisé habituellement avec \tikzlg{} pour des raisons de compatibilité avec \href{https://ctan.org/pkg/beamer}{\color{black}\textsc{beamer}}}
\subsection{\boldmath Ne pas importer les commandes avec $\mcdot$}
Il est possible de ne pas modifier les commandes usuelles en utilisant l'option \Verb?nodots? de ce package. Il faut alors importer le package avec \Verb?\usepackage[nodots]{matrices}?.
\subsection{\boldmath Modifier les séparations entre les $\mcdot$}
Les commandes avec des points tel que $\xdots$ ont des définitions qui dépendent de la taille de la police (celle pour \LaTeX~est adaptée pour 12pt, et celle de \href{https://ctan.org/pkg/beamer}{\color{black}\textsc{beamer}} pour 17pt). Pour avoir des points alignés correctement, il est possible de modifier la valeur de \Verb?\dotsep? en utilisant \Verb?\setlength{\dotsep}{taille_en_pt}?.

Par exemple, avec 2pt, on obtient : \setlength{\dotsep}{2pt} $\xdots$.\setlength{\dotsep}{3.5pt}

Il est également possible de faire de même avec la hauteur des $\mdots$ en modifiant la longueur \Verb?\dotlift?. De même avec \Verb?\matmin? pour \texttt{minimum width} et \texttt{minimum width}, ou encore \Verb?\matsep? pour \texttt{row sep} et \texttt{column sep}.
\subsection{La commande \texttt{\textbackslash tmatrix}}
...
\subsection{Exemples avec \texttt{\textbackslash tmatrix}}
...
\mcmd{\poiss{\lambda}}
\hsection{Sffont.sty}
Ce package définit une nouvelle police \texttt{cmssp} qui correspond à \texttt{cmss} en 10pt. Pour l'utiliser, il faut utiliser \Verb?\fontfamily{cmssp}\fontsize{Xpt}{\baselineskip}\selectfont?.

Exemples avec \texttt{cmssp} :

{\bfseries\fontfamily{cmssp}\fontsize{25pt}{\baselineskip}\selectfont Exemple avec une police de taille 25pt en gras.}

{\itshape\fontfamily{cmssp}\fontsize{17pt}{\baselineskip}\selectfont Exemple avec une police de taille 17pt en italique.}

{\itshape\bfseries\fontfamily{cmssp}\fontsize{12pt}{\baselineskip}\selectfont Exemple avec une police de taille 12pt en gras italique.}

Les mêmes exemples avec \texttt{cmss} :

{\bfseries\fontfamily{cmss}\fontsize{25pt}{\baselineskip}\selectfont Exemple avec une police de taille 25pt en gras.}

{\itshape\fontfamily{cmss}\fontsize{17pt}{\baselineskip}\selectfont Exemple avec une police de taille 17pt en italique.}

{\itshape\bfseries\fontfamily{cmss}\fontsize{12pt}{\baselineskip}\selectfont Exemple avec une police de taille 12pt en gras italique.}
...
Il est possible d'utiliser \Verb?[ampersand replacement=\&]? puis \Verb?\&? pour la matrice lorsque \Verb?&? est déjà défini par .
...
\footnotetext{Correspond à la commande usuelle \Verb?\min?}
\mcmd{\min{\llb0,n\rrb}}
\mcmd{\min[\mathbb{N}^*]{\llb0,n\rrb}}
\mcmd[\footnotemark]{\oldmax}
\footnotetext{Correspond à la commande usuelle \Verb?\max?}
\mcmd{\max{\llb0,n\rrb}}
\mcmd{\max[\mathbb{Z}_-]{\llb0,n\rrb}}
\mcmd[\footnotemark]{\oldlim}
\footnotetext{Correspond à la commande usuelle \Verb?\lim?}
\mcmd{\lim{u_n}}
...
\footnotetext{Correspond à la commande usuelle \Verb?\exp?}
\mcmd{\exp{x}}
\mcmd{\exp[n]{x}}
\mcmd[\footnotemark]{\oldln}
\footnotetext{Correspond à la commande usuelle \Verb?\ln?}
\mcmd{\ln{x}}
\mcmd{\ln[n]{x}}
\mcmd[\footnotemark]{\oldinf}
\footnotetext{Correspond à la commande usuelle \Verb?\inf?}
\mcmd{\inf{\varnothing}}
\mcmd{\inf[\bar{\mathbb{R}}]{\{u_n\}}}
\mcmd[\footnotemark]{\oldsup}
\footnotetext{Correspond à la commande usuelle \Verb?\sup?}
\mcmd{\sup{\varnothing}}
