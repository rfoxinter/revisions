...
Si {\color{namecolour}\texttt{nopar}} est utilisé, les commandes comme \mintinline{latexmath}?\der?, \mintinline{latexmath}?\pder?, \mintinline{latexmath}?\mpder?, et \mintinline{latexmath}?\int? n'ont plus de parenthèses automatiques. Avec cette option, on a \mintinline{latexmath}?\der{}{f(x)}? qui produit <<~$\frac{{\dd}}{{\dd}x}f(x)$~>> et \mintinline{latexmath}?\int[x]{f(x)}? qui produit <<~$\displaystyle{\oldint}f(x){{}\dd}x$~>>.
...
\subsection{L'option {\color{namecolour}\texttt{nopar}}}
Si {\color{namecolour}\texttt{nopar}} est utilisé, les commandes n'ont plus de parenthèses automatiques. Avec cette option, on a \mintinline{latexmath}?\sum{n=0}{+\infty}{u_n}? qui produit <<~$\displaystyle{\oldsum_{n=0}^{+\infty}}u_n$~>>.
