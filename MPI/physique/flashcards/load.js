...
function display_files(sitemap){
    const path = 'https://rfoxinter.github.io/revisions/MPI/physique/pdf/';
    const extension = '\\.pdf';

    var files = [];

    while (sitemap.search(path)!=-1) {
        var idx_start = sitemap.search(path)+55;
    ...
    for (var i=0; i<l; ++i) {
        ...
        if (filename.search(/\.[0-9]/g) == f.length-2) {
            ...
            if (name_tmp != name) {
                ...
                a.setAttribute('href', '../../../flashcards/?file=' + btoa('../MPI/physique/flashcards/' + name_tmp).replaceAll('=', ''));
            ...
            var a = document.createElement('a');
            a.setAttribute('href', '../../../flashcards/?file=' + btoa('../MPI/physique/flashcards/' + f).replaceAll('=', ''));
            a.setAttribute('target', '_blank');
            span = document.createElement('span');
            span.setAttribute('class','material-symbols-rounded');
            span.innerHTML = 'open_in_new';
