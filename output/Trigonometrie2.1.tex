...
\slideq{$\der{\ash{x}}$}{2/84}
\slider{$\frac{1}{\sqrt{x^2+1}}$}{2/84}
\slideq{$\der{\ach{x}}$}{3/84}
\slider{$\frac{1}{\sqrt{x^2-1}}$}{3/84}
\slideq{$-2\sin{\frac{x+y}{2}}\sin{\frac{x-y}{2}}$}{4/84}
\slider{$\cos{x}-\cos{y}$}{4/84}
\slideq{$\sin{\frac{\pi}{2}-x}$}{5/84}
\slider{$\cos{x}$}{5/84}
\slideq{$\lim{x\to0}{\frac{\sh{x}}{x}}$}{6/84}
\slider{$1$}{6/84}
...
\slideq{$\cot{2x}$}{8/84}
\slider{$\frac{\cot[2]{x}-1}{2\cot{x}}$}{8/84}
\slideq{$n\in\N^*$, $x\in\R$\linebreak$\der[n]{\sin{x}}$}{9/84}
\slider{$\sin{x+\frac{n\pi}{2}}=\left\{\begin{matrix*}[l]\l-1\r^{\left\lfloor\oldfrac{n}{2}\right\rfloor}\sin{x}&\;\text{si }n\in2\N\\\l-1\r^{\left\lfloor\oldfrac{n}{2}\right\rfloor}\cos{x}&\;\text{sinon}\end{matrix*}\right.\!$}{9/84}
\slideq{$\lim{x\to0}{\frac{\sin{x}}{x}}$}{10/84}
\slider{1}{10/84}
\slideq{$\sin{x+\frac{\pi}{2}}$}{11/84}
\slider{$\cos{x}$}{11/84}
\slideq{$\der{\asin{x}}$}{12/84}
\slider{$\frac{1}{\sqrt{1-x^2}}$}{12/84}
\slideq{$\tan{2x}$}{13/84}
\slider{$\frac{2\tan{x}}{1-\tan[2]{x}}$}{13/84}
\slideq{$\cos{\pi-x}$}{14/84}
\slider{$-\cos{x}$}{14/84}
\slideq{$\cos{x}\cos{y}-\sin{x}\sin{y}$}{15/84}
\slider{$\cos{x+y}$}{15/84}
\slideq{$\ach{x}$}{16/84}
\slider{$\ln{x+\sqrt{x^2-1}}$}{16/84}
\slideq{$\frac{1}{2}\l\sin{x+y}-\sin{x-y}\r$}{17/84}
\slider{$\cos{x}\sin{y}$}{17/84}
\slideq{$\lim{x\to0}{\frac{\ch{x}-1}{x^2}}$}{18/84}
\slider{$\frac{1}{2}$}{18/84}
\slideq{$\tan{x}$\linebreak\linebreak$t=\tan{\frac{x}{2}}$}{19/84}
\slider{$\frac{2t}{1-t^2}$\linebreak\linebreak$t=\tan{\frac{x}{2}}$}{19/84}
\slideq{$\cos{x+\pi}$}{20/84}
\slider{$-\cos{x}$}{20/84}
\slideq{$\sin{x}\cos{y}+\sin{y}\cos{x}$}{21/84}
\slider{$\sin{x+y}$}{21/84}
\slideq{$\sin{x}+\sin{y}$}{22/84}
\slider{$2\sin{\frac{x+y}{2}}\cos{\frac{x-y}{2}}$}{22/84}
\slideq{$n\in\N^*$, $x\in\R$\linebreak$\der[n]{\cos{x}}$}{23/84}
\slider{$\cos{x+\frac{n\pi}{2}}=\left\{\begin{matrix*}[l]\l-1\r^{\left\lfloor\oldfrac{n}{2}\right\rfloor}\cos{x}&\;\text{si }n\in2\N\\\l-1\r^{\left\lfloor\oldfrac{n}{2}\right\rfloor+1}\sin{x}&\;\text{sinon}\end{matrix*}\right.\!$}{23/84}
\slideq{Argument d'un nombre complexe $z$ avec $\atan{x}$}{24/84}
\slider{$\arg\l z\r\equiv\atan{\frac{\Im\l z\r}{\Re\l z\r}}\,\left[\pi\right]$}{24/84}
\slideq{Courbe représentative de $\ach{x}$}{25/84}
\slider{\begin{tikzpicture} \draw[color=gray,opacity=0.2] (-0.2,-0.2) grid (10.2,3.2); \draw[->] (-0.2, 0) -- (10.2, 0) node[above left]{$x$}; \draw[->] (0,-0.2) -- (0, 3.2) node[below right]{$y$}; \foreach \y in {1,...,3} \draw(0,\y) node[left]{\footnotesize$\y$}; \foreach \x in {1,...,10} \draw(\x,0) node[below]{\footnotesize$\x$}; \draw(0,0) node[below left]{\footnotesize$0$}; \draw[domain=1:10.2, samples=1100, scale=1, smooth, variable=\x, black] plot (\x,{ln(\x+sqrt(\x*\x-1))}); \node[above] at (current bounding box.north) {$y=\ach{x}$}; \end{tikzpicture}}{25/84}
\slideq{$\lim{x\to0}{\frac{\tan{x}}{x}}$}{26/84}
\slider{$1$}{26/84}
\slideq{$\cos{\frac{\pi}{2}-x}$}{27/84}
\slider{$\sin{x}$}{27/84}
\slideq{Courbe représentative de $\th{x}$}{28/84}
\slider{\begin{tikzpicture} \draw[color=gray,opacity=0.2] (-5.2,-1.2) grid (5.2,1.2); \draw[->] (-5.2, 0) -- (5.2, 0) node[above left]{$x$}; \draw[->] (0,-1.2) -- (0, 1.2) node[below right]{$y$}; \foreach \y in {-1,1} \draw(0,\y) node[left]{\footnotesize$\y$}; \foreach \x in {-5,...,-1} \draw(\x,0) node[below]{\footnotesize$\x$}; \foreach \x in {1,...,5} \draw(\x,0) node[below]{\footnotesize$\x$}; \draw(0,0) node[below left]{\footnotesize$0$}; \draw[domain=-5.2:5.2, samples=1100, scale=1, smooth, variable=\x, black] plot (\x,{(exp(\x)-exp(-\x))/(exp(\x)+exp(-\x))}); \node[above] at (current bounding box.north) {$y=\th{x}$}; \end{tikzpicture}}{28/84}
\slideq{$\frac{1}{2}\l\cos{x+y}+\cos{x-y}\r$}{29/84}
\slider{$\cos{x}\cos{y}$}{29/84}
\slideq{$\lim{x\to0}{\frac{1-\cos{x}}{x^2}}$}{30/84}
\slider{$\frac{1}{2}$}{30/84}
\slideq{$\der{\tan{x}}$}{31/84}
\slider{$\frac{1}{\cos[2]{x}}=1+\tan[2]{x}$}{31/84}
\slideq{Inégalités classiques de $\sin{x}$}{32/84}
\slider{$\forall x\in\R_+,\;\sin{x}\leqslant x$\linebreak$\forall x\in\R_-,\;\sin{x}\geqslant x$\linebreak$\forall x\in\R,\;\left|\sin{x}\right|\leqslant\left|x\right|$\linebreak$\forall x\in\left[0,\oldfrac{\pi}{2}\right],\;\sin{x}\geqslant\oldfrac{2x}{\pi}$}{32/84}
\slideq{$\lim{x\to0}{\frac{\asin{x}}{x}}$}{33/84}
\slider{$1$}{33/84}
\slideq{$\lim{x\to0}{\frac{\th{x}}{x}}$}{34/84}
\slider{$1$}{34/84}
\slideq{$\cos{x+\frac{\pi}{2}}$}{35/84}
\slider{$-\sin{x}$}{35/84}
\slideq{Inégalité classique de $\atan{x}$}{36/84}
\slider{$\forall x\in\R,\;\left|\atan{x}\right|\leqslant\left|x\right|$}{36/84}
...
\slideq{$\frac{1-t^2}{1+t^2}$\linebreak\linebreak$t=\tan{\frac{x}{2}}$}{53/84}
\slider{$\cos{x}$\linebreak\linebreak$t=\tan{\frac{x}{2}}$}{53/84}
\slideq{$\sin{x+\pi}$}{54/84}
\slider{$-\sin{x}$}{54/84}
\slideq{$\cos{x}\cos{y}+\sin{x}\sin{y}$}{55/84}
\slider{$\cos{x-y}$}{55/84}
\slideq{Propriétés remarquables de $\acos{x}$}{56/84}
\slider{$\forall x\in\left[-1,1\right],\;\cos{\acos{x}}=x$\linebreak$\forall x\in\left[0,\pi\right],\;\acos{\cos{x}}=x$\linebreak$\forall x\in\left[-\pi,0\right],\;\acos{\cos{x}}=-x$}{56/84}
\slideq{$\lim{x\to0}{\frac{\atan{x}}{x}}$}{57/84}
\slider{$1$}{57/84}
\slideq{$\ash{x}$}{58/84}
\slider{$\ln{x+\sqrt{x^2+1}}$}{58/84}
\slideq{$\tan{x-y}$}{59/84}
\slider{$\frac{\tan{x}-\tan{y}}{1+\tan{x}\tan{y}}$}{59/84}
\slideq{$\cos[2]{x}$}{60/84}
\slider{$\frac{1+\cos{2x}}{2}$}{60/84}
\slideq{Courbe représentative de $\atan{x}$}{61/84}
\slider{\begin{tikzpicture} \draw[color=gray,opacity=0.2] (-5.2,-2.2) grid[ystep=pi/4](5.2,2.2); \draw[->] (-5.2, 0) -- (5.2, 0) node[above left]{$x$}; \draw[->] (0,-2.2) -- (0, 2.2) node[below right]{$y$}; \foreach \y[count=\cnt] in {-\frac{\pi}{4},-\frac{\pi}{2}} \draw(0,{(-\cnt/4)*pi}) node[left]{\footnotesize$\y$}; \foreach \y[count=\cnt] in {\frac{\pi}{4},\frac{\pi}{2}} \draw(0,{(\cnt/4)*pi}) node[left]{\footnotesize$\y$}; \foreach \x in {-5,...,-1} \draw(\x,0) node[below]{\footnotesize$\x$}; \foreach \x in {1,...,5} \draw(\x,0) node[below]{\footnotesize$\x$}; \draw(0,0) node[below left]{\footnotesize$0$}; \draw[domain=-5.2:5.2, samples=2000, scale=1, smooth, variable=\x, black] plot (\x,{rad(atan(\x))}); \node[above] at (current bounding box.north) {$y=\atan{x}$}; \end{tikzpicture}}{61/84}
\slideq{$\der{\atan{x}}$}{62/84}
\slider{$\frac{1}{1+x^2}$}{62/84}
\slideq{$\lim{x\to+\infty}{\th{x}}$}{63/84}
\slider{$1$}{63/84}
\slideq{$\sin{x}\cos{y}-\sin{y}\cos{x}$}{64/84}
\slider{$\sin{x-y}$}{64/84}
\slideq{$n\in\N^*$, $x\in\R$\linebreak$\der[n]{\ch{x}}$}{65/84}
\slider{$\left\{\begin{matrix*}[l]\ch{x}&\;\text{si }n\in2\N\\\sh{x}&\;\text{sinon}\end{matrix*}\right.\!$}{65/84}
\slideq{$\cot{x-y}$}{66/84}
\slider{$\frac{-\cot{x}\cot{y}-1}{\cot{x}-\cot{y}}=\frac{\cot{x}\cot{y}+1}{\cot{y}-\cot{x}}$}{66/84}
\slideq{$2\sin{x}\cos{x}$}{67/84}
\slider{$\sin{2x}$}{67/84}
\slideq{$2\cos{\frac{x+y}{2}}\cos{\frac{x-y}{2}}$}{68/84}
\slider{$\cos{x}+\cos{y}$}{68/84}
\slideq{$\frac{\cot{x}\cot{y}-1}{\cot{x}+\cot{y}}$}{69/84}
\slider{$\cot{x+y}$}{69/84}
\slideq{$\sin[2]{x}$}{70/84}
\slider{$\frac{1-\cos{2x}}{2}$}{70/84}
\slideq{Courbe représentative de $\sh{x}$}{71/84}
\slider{\begin{tikzpicture} \draw[color=gray,opacity=0.2] (-2.2,-2.7) grid (2.2,2.7); \draw[->] (-2.2, 0) -- (2.2, 0) node[above left]{$x$}; \draw[->] (0,-2.7) -- (0, 2.7) node[below right]{$y$}; \foreach \y in {-2,-1,1,2} \draw(0,\y) node[left]{\footnotesize$\y$}; \foreach \x in {-2,-1} \draw(\x,0) node[below]{\footnotesize$\x$}; \foreach \x in {1,2} \draw(\x,0) node[below]{\footnotesize$\x$}; \draw(0,0) node[below left]{\footnotesize$0$}; \draw[domain=-1.72:1.72, samples=400, scale=1, smooth, variable=\x, black] plot (\x,{(exp(\x)-exp(-\x))/2}); \node[above] at (current bounding box.north) {$y=\sh{x}$}; \end{tikzpicture}}{71/84}
