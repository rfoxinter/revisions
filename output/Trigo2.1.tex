...
\slideq{$\frac{1}{2}\l\cos{x-y}-\cos{x+y}\r$}{21/84}
\slider{$\sin{x}\sin{y}$}{21/84}
\slideq{$\cos{\pi-x}$}{22/84}
\slider{$-\cos{x}$}{22/84}
\slideq{Expression de $\ach{x}$}{23/84}
\slider{$\ln{x+\sqrt{x^2-1}}$}{23/84}
\slideq{$n\in\N^*$, $x\in\R$\linebreak$\der[n]{\sh{x}}$}{24/84}
\slider{$\left\{\begin{matrix*}[l]\sh{x}&\;\text{si }n\in2\N\\\ch{x}&\;\text{sinon}\end{matrix*}\right.\!$}{24/84}
\slideq{$\cos[2]{x}-\sin[2]{x}=2\cos[2]{x}-1=1-2\sin[2]{x}$}{25/84}
\slider{$\cos{2x}$}{25/84}
\slideq{Argument d'un nombre complexe $z$ avec $\atan{x}$}{26/84}
\slider{$\arg\l z\r\equiv\atan{\frac{\Im\l z\r}{\Re\l z\r}}\,\left[\pi\right]$}{26/84}
\slideq{Propriété remarquable de $\acos{x}$ et $\asin{x}$}{27/84}
\slider{$\acos{x}+\asin{x}=\frac{\pi}{2}$}{27/84}
\slideq{$\sin{x}$\linebreak\linebreak$t=\tan{\frac{x}{2}}$}{28/84}
\slider{$\frac{2t}{1+t^2}$\linebreak\linebreak$t=\tan{\frac{x}{2}}$}{28/84}
\slideq{$\cos{x+\frac{\pi}{2}}$}{29/84}
\slider{$-\sin{x}$}{29/84}
\slideq{Courbe représentative de $\cos{x}$}{30/84}
\slider{\begin{tikzpicture} \draw[color=gray,opacity=0.2] (-5.2,-1.2) grid[xstep=pi/2](5.2,1.2); \draw[->] (-5.2, 0) -- (5.2, 0) node[above left]{$x$}; \draw[->] (0,-1.2) -- (0, 1.2) node[below right]{$y$}; \foreach \y in {-1} \draw(0,\y) node[left]{\footnotesize$\y$}; \foreach \y in {1} \draw(0,\y) node[left]{\footnotesize$\y$}; \foreach \x[count=\cnt] in {-\frac{\pi}{2},-\pi,-\frac{3\pi}{2}} \draw({(-\cnt/2)*pi},0) node[below]{\footnotesize$\x$}; \foreach \x[count=\cnt] in {\frac{\pi}{2},\pi,\frac{3\pi}{2}} \draw({(\cnt/2)*pi},0) node[below]{\footnotesize$\x$}; \draw(0,0) node[below left]{\footnotesize$0$}; \draw[domain=-5.2:5.2, samples=1000, smooth, variable=\x, black] plot (\x, {cos(\x r)}); \node[above] at (current bounding box.north) {$y=\cos{x}$};\end{tikzpicture}}{30/84}
\slideq{$\cos{x-y}$}{31/84}
\slider{$\cos{x}\cos{y}+\sin{x}\sin{y}$}{31/84}
\slideq{Inégalités classiques de $\sh{x}$}{32/84}
\slider{$\forall x\in\R_+,\;\sh{x}\geqslant x$\linebreak$\forall x\in\R_-,\;\sh{x}\leqslant x$}{32/84}
\slideq{Valeurs remarquables}{33/84}
...
\slideq{$\lim{x\to0}{\frac{\sin{x}}{x}}$}{49/84}
\slider{1}{49/84}
\slideq{Propriétés remarquables de $\acos{x}$}{50/84}
\slider{$\forall x\in\left[-1,1\right],\;\cos{\acos{x}}=x$\linebreak$\forall x\in\left[0,\pi\right],\;\acos{\cos{x}}=x$\linebreak$\forall x\in\left[-\pi,0\right],\;\acos{\cos{x}}=-x$}{50/84}
\slideq{$-2\sin{\frac{x+y}{2}}\sin{\frac{x-y}{2}}$}{51/84}
\slider{$\cos{x}-\cos{y}$}{51/84}
\slideq{$\der{\ath{x}}$}{52/84}
\slider{$\frac{1}{1-x^2}$}{52/84}
\slideq{$\frac{1+\cos{2x}}{2}$}{53/84}
\slider{$\cos[2]{x}$}{53/84}
\slideq{$\der{\atan{x}}$}{54/84}
\slider{$\frac{1}{1+x^2}$}{54/84}
\slideq{$\lim{x\to0}{\frac{1-\cos{x}}{x^2}}$}{55/84}
\slider{$\frac{1}{2}$}{55/84}
\slideq{$n\in\N^*$, $x\in\R$\linebreak$\der[n]{\ch{x}}$}{56/84}
\slider{$\left\{\begin{matrix*}[l]\ch{x}&\;\text{si }n\in2\N\\\sh{x}&\;\text{sinon}\end{matrix*}\right.\!$}{56/84}
\slideq{$\lim{x\to0}{\frac{\atan{x}}{x}}$}{57/84}
\slider{$1$}{57/84}
\slideq{$\sin{x+\pi}$}{58/84}
\slider{$-\sin{x}$}{58/84}
\slideq{Inégalité classique de $\atan{x}$}{59/84}
\slider{$\forall x\in\R,\;\left|\atan{x}\right|\leqslant\left|x\right|$}{59/84}
\slideq{$\tan{x}$\linebreak\linebreak$t=\tan{\frac{x}{2}}$}{60/84}
\slider{$\frac{2t}{1-t^2}$\linebreak\linebreak$t=\tan{\frac{x}{2}}$}{60/84}
\slideq{Valeurs remarquables de $\atan{x}$}{61/84}
...
\slideq{$\frac{-\cot{x}\cot{y}-1}{\cot{x}-\cot{y}}=\frac{\cot{x}\cot{y}+1}{\cot{y}-\cot{x}}$}{62/84}
\slider{$\cot{x-y}$}{62/84}
\slideq{$\sin{\pi-x}$}{63/84}
\slider{$\sin{x}$}{63/84}
\slideq{$\lim{x\to0}{\frac{\ch{x}-1}{x^2}}$}{64/84}
\slider{$\frac{1}{2}$}{64/84}
\slideq{$\der{\acos{x}}$}{65/84}
\slider{$-\frac{1}{\sqrt{1-x^2}}$}{65/84}
\slideq{Expression de $\ash{x}$}{66/84}
\slider{$\ln{x+\sqrt{x^2+1}}$}{66/84}
\slideq{$\lim{x\to0}{\frac{\asin{x}}{x}}$}{67/84}
\slider{$1$}{67/84}
\slideq{Courbe représentative de $\ash{x}$}{68/84}
\slider{\begin{tikzpicture} \draw[color=gray,opacity=0.2] (-5.2,-2.7) grid (5.2,2.7); \draw[->] (-5.2, 0) -- (5.2, 0) node[above left]{$x$}; \draw[->] (0,-2.7) -- (0, 2.7) node[below right]{$y$}; \foreach \y in {-2,-1,1,2} \draw(0,\y) node[left]{\footnotesize$\y$}; \foreach \x in {-5,...,-1} \draw(\x,0) node[below]{\footnotesize$\x$}; \foreach \x in {1,...,5} \draw(\x,0) node[below]{\footnotesize$\x$}; \draw(0,0) node[below left]{\footnotesize$0$}; \draw[domain=0:5.2, samples=600, scale=1, smooth, variable=\x, black] plot (\x,{ln(\x+sqrt(\x*\x+1))}); \draw[domain=0:5.2, samples=600, scale=1, smooth, variable=\x, black] plot (-\x,{-ln(\x+sqrt(\x*\x+1))}); \node[above] at (current bounding box.north) {$y=\ash{x}$}; \end{tikzpicture}}{68/84}
\slideq{Inégalités classiques de $\sin{x}$}{69/84}
\slider{$\forall x\in\R_+,\;\sin{x}\leqslant x$\linebreak$\forall x\in\R_-,\;\sin{x}\geqslant x$\linebreak$\forall x\in\R,\;\left|\sin{x}\right|\leqslant\left|x\right|$\linebreak$\forall x\in\left[0,\oldfrac{\pi}{2}\right],\;\sin{x}\geqslant\oldfrac{2x}{\pi}$}{69/84}
\slideq{$\lim{x\to+\infty}{\th{x}}$}{70/84}
\slider{$1$}{70/84}
\slideq{$\sin{x+\frac{\pi}{2}}$}{71/84}
\slider{$\cos{x}$}{71/84}
\slideq{$2\sin{\frac{x+y}{2}}\cos{\frac{x-y}{2}}$}{72/84}
\slider{$\sin{x}+\sin{y}$}{72/84}
\slideq{$\lim{x\to0}{\frac{\th{x}}{x}}$}{73/84}
\slider{$1$}{73/84}
\slideq{$\der{\tan{x}}$}{74/84}
\slider{$\frac{1}{\cos[2]{x}}=1+\tan[2]{x}$}{74/84}
\slideq{$\cot{x+y}$}{75/84}
\slider{$\frac{\cot{x}\cot{y}-1}{\cot{x}+\cot{y}}$}{75/84}
\slideq{$\frac{1-\cos{2x}}{2}$}{76/84}
\slider{$\sin[2]{x}$}{76/84}
\slideq{$\der{\ach{x}}$}{77/84}
\slider{$\frac{1}{\sqrt{x^2-1}}$}{77/84}
\slideq{$\tan{x+y}$}{78/84}
\slider{$\frac{\tan{x}+\tan{y}}{1-\tan{x}\tan{y}}$}{78/84}
\slideq{$\sin{2x}$}{79/84}
\slider{$2\sin{x}\cos{x}$}{79/84}
\slideq{$\cos{x}\sin{y}$}{80/84}
\slider{$\frac{1}{2}\l\sin{x+y}-\sin{x-y}\r$}{80/84}
\slideq{$\sin{x}\cos{y}+\sin{y}\cos{x}$}{81/84}
\slider{$\sin{x+y}$}{81/84}
\slideq{$\sin{x-y}$}{82/84}
\slider{$\sin{x}\cos{y}-\sin{y}\cos{x}$}{82/84}
\slideq{$\cos{x+\pi}$}{83/84}
\slider{$-\cos{x}$}{83/84}
\slideq{$\cos{x}\cos{y}-\sin{x}\sin{y}$}{84/84}
\slider{$\cos{x+y}$}{84/84}
\end{document}