...
\slideq{$n\in\N^*$, $x\in\R$\linebreak$\der[n]{}{\ch{x}}$}{9/84}
\slider{$\left\{\begin{matrix*}[l]\ch{x}&\;\text{si }n\in2\N\\\sh{x}&\;\text{sinon}\end{matrix*}\right.\!$}{9/84}
\slideq{$2\cos{\frac{x+y}{2}}\sin{\frac{x-y}{2}}$}{10/84}
\slider{$\sin{x}-\sin{y}$}{10/84}
\slideq{Valeurs remarquables de $\asin{x}$ et $\acos{x}$}{11/84}
...
\slideq{$\frac{2t}{1-t^2}$\linebreak\linebreak$t=\tan{\frac{x}{2}}$}{18/84}
\slider{$\tan{x}$\linebreak\linebreak$t=\tan{\frac{x}{2}}$}{18/84}
\slideq{$\der{}{\ath{x}}$}{19/84}
\slider{$\frac{1}{1-x^2}$}{19/84}
...
\slideq{$\der{}{\acos{x}}$}{21/84}
\slider{$-\frac{1}{\sqrt{1-x^2}}$}{21/84}
\slideq{$\sin{x}\cos{y}+\sin{y}\cos{x}$}{22/84}
\slider{$\sin{x+y}$}{22/84}
\slideq{$\cos{x}\cos{y}$}{23/84}
\slider{$\frac{1}{2}\l\cos{x+y}+\cos{x-y}\r$}{23/84}
\slideq{$\frac{2\tan{x}}{1-\tan[2]{x}}$}{24/84}
\slider{$\tan{2x}$}{24/84}
\slideq{Propriété remarquable de $\acos{x}$ et $\asin{x}$}{25/84}
\slider{$\acos{x}+\asin{x}=\frac{\pi}{2}$}{25/84}
\slideq{$\lim{x\to0}{\frac{\ch{x}-1}{x^2}}$}{26/84}
\slider{$\frac{1}{2}$}{26/84}
\slideq{$\lim{x\to0}{\frac{\th{x}}{x}}$}{27/84}
\slider{$1$}{27/84}
\slideq{Propriétés remarquables de $\asin{x}$}{28/84}
\slider{$\forall x\in\left[-1,1\right],\;\sin{\asin{x}}=x$\linebreak$\forall x\in\left[-\oldfrac{\pi}{2},\oldfrac{\pi}{2}\right],\;\asin{\sin{x}}=x$\linebreak$\forall x\in\left[\oldfrac{\pi}{2},\oldfrac{3\pi}{2}\right],\;\asin{\sin{x}}=\pi-x$}{28/84}
\slideq{$\lim{x\to0}{\frac{\sh{x}}{x}}$}{29/84}
\slider{$1$}{29/84}
\slideq{$\lim{x\to0}{\frac{1-\cos{x}}{x^2}}$}{30/84}
\slider{$\frac{1}{2}$}{30/84}
\slideq{$\cos{\pi-x}$}{31/84}
\slider{$-\cos{x}$}{31/84}
\slideq{$\der{}{\ach{x}}$}{32/84}
\slider{$\frac{1}{\sqrt{x^2-1}}$}{32/84}
\slideq{$\cos{x}\cos{y}+\sin{x}\sin{y}$}{33/84}
\slider{$\cos{x-y}$}{33/84}
\slideq{$\sin{\frac{\pi}{2}-x}$}{34/84}
\slider{$\cos{x}$}{34/84}
\slideq{$\cot{2x}$}{35/84}
\slider{$\frac{\cot[2]{x}-1}{2\cot{x}}$}{35/84}
\slideq{Courbe représentative de $\acos{x}$}{36/84}
\slider{\begin{tikzpicture} \draw[color=gray,opacity=0.2] (-1.2,-0.2) grid[ystep=pi/2](1.2,3.2); \draw[->] (-1.2, 0) -- (1.2, 0) node[above left]{$x$}; \draw[->] (0,-0.2) -- (0, 3.2) node[below right]{$y$}; \foreach \y[count=\cnt] in {\frac{\pi}{2},\pi} \draw(0,{(\cnt/2)*pi}) node[left]{\footnotesize$\y$}; \foreach \x in {-1} \draw(\x,0) node[below]{\footnotesize$\x$}; \foreach \x in {1} \draw(\x,0) node[below]{\footnotesize$\x$}; \draw(0,0) node[below left]{\footnotesize$0$}; \draw[domain=pi:0, samples=400, scale=1, smooth, variable=\x, black] plot ({cos(\x r)},\x); \node[above] at (current bounding box.north) {$y=\acos{x}$}; \end{tikzpicture}}{36/84}
\slideq{$\frac{2t}{1+t^2}$\linebreak\linebreak$t=\tan{\frac{x}{2}}$}{37/84}
\slider{$\sin{x}$\linebreak\linebreak$t=\tan{\frac{x}{2}}$}{37/84}
\slideq{$\frac{\tan{x}+\tan{y}}{1-\tan{x}\tan{y}}$}{38/84}
\slider{$\tan{x+y}$}{38/84}
\slideq{Courbe représentative de $\asin{x}$}{39/84}
\slider{\begin{tikzpicture} \draw[color=gray,opacity=0.2] (-1.2,-2.2) grid[ystep=pi/2](1.2,2.2); \draw[->] (-1.2, 0) -- (1.2, 0) node[above left]{$x$}; \draw[->] (0,-2.2) -- (0, 2.2) node[below right]{$y$}; \foreach \y[count=\cnt] in {-\frac{\pi}{2}} \draw(0,{(-\cnt/2)*pi}) node[left]{\footnotesize$\y$}; \foreach \y[count=\cnt] in {\frac{\pi}{2}} \draw(0,{(\cnt/2)*pi}) node[left]{\footnotesize$\y$}; \foreach \x in {-1} \draw(\x,0) node[below]{\footnotesize$\x$}; \foreach \x in {1} \draw(\x,0) node[below]{\footnotesize$\x$}; \draw(0,0) node[below left]{\footnotesize$0$}; \draw[domain=-pi/2:pi/2, samples=400, scale=1, smooth, variable=\x, black] plot ({sin(\x r)},\x); \node[above] at (current bounding box.north) {$y=\asin{x}$}; \end{tikzpicture}}{39/84}
\slideq{$\der{}{\th{x}}$}{40/84}
\slider{$1-\th[2]{x}=\frac{1}{\ch[2]{x}}$}{40/84}
\slideq{$\der{}{\asin{x}}$}{41/84}
\slider{$\frac{1}{\sqrt{1-x^2}}$}{41/84}
\slideq{Courbe représentative de $\ch{x}$}{42/84}
\slider{\begin{tikzpicture} \draw[color=gray,opacity=0.2] (-2.7,-0.2) grid (2.7,5.2); \draw[->] (-2.7, 0) -- (2.7, 0) node[above left]{$x$}; \draw[->] (0,-0.7) -- (0, 5.2) node[below right]{$y$}; \foreach \y in {1,...,5} \draw(0,\y) node[left]{\footnotesize$\y$}; \foreach \x in {-2,-1} \draw(\x,0) node[below]{\footnotesize$\x$}; \foreach \x in {1,2} \draw(\x,0) node[below]{\footnotesize$\x$}; \draw(0,0) node[below left]{\footnotesize$0$}; \draw[domain=-2.333:2.333, samples=500, scale=1, smooth, variable=\x, black] plot (\x,{(exp(\x)+exp(-\x))/2}); \node[above] at (current bounding box.north) {$y=\ch{x}$}; \end{tikzpicture}}{42/84}
\slideq{$\der{}{\ash{x}}$}{43/84}
\slider{$\frac{1}{\sqrt{x^2+1}}$}{43/84}
\slideq{$\cos{\frac{\pi}{2}-x}$}{44/84}
\slider{$\sin{x}$}{44/84}
\slideq{Propriétés remarquables de $\atan{x}$}{45/84}
\slider{$\forall x\in\R,\;\tan{\atan{x}}=x$\linebreak$\forall x\in\left]-\oldfrac{\pi}{2},\oldfrac{\pi}{2}\right[,\;\atan{\tan{x}}=x$\linebreak$\forall n\in\N,\;\forall x\in\left]-\oldfrac{\pi}{2}+n\pi,\oldfrac{\pi}{2}+n\pi\right[$\linebreak$\atan{\tan{x}}=x-n\pi$\linebreak$\forall x\in\R^*,\;\atan{x}+\atan{\oldfrac{1}{x}}=\oldfrac{x}{\left|x\right|}\oldfrac{\pi}{2}$}{45/84}
\slideq{$n\in\N^*$, $x\in\R$\linebreak$\der[n]{}{\sin{x}}$}{46/84}
\slider{$\sin{x+\frac{n\pi}{2}}=\left\{\begin{matrix*}[l]\l-1\r^{\left\lfloor\oldfrac{n}{2}\right\rfloor}\sin{x}&\;\text{si }n\in2\N\\\l-1\r^{\left\lfloor\oldfrac{n}{2}\right\rfloor}\cos{x}&\;\text{sinon}\end{matrix*}\right.\!$}{46/84}
\slideq{$n\in\N^*$, $x\in\R$\linebreak$\der[n]{}{\cos{x}}$}{47/84}
\slider{$\cos{x+\frac{n\pi}{2}}=\left\{\begin{matrix*}[l]\l-1\r^{\left\lfloor\oldfrac{n}{2}\right\rfloor}\cos{x}&\;\text{si }n\in2\N\\\l-1\r^{\left\lfloor\oldfrac{n}{2}\right\rfloor+1}\sin{x}&\;\text{sinon}\end{matrix*}\right.\!$}{47/84}
...
\slideq{$\cos[2]{x}-\sin[2]{x}=2\cos[2]{x}-1=1-2\sin[2]{x}$}{50/84}
\slider{$\cos{2x}$}{50/84}
\slideq{$\cot{x-y}$}{51/84}
\slider{$\frac{-\cot{x}\cot{y}-1}{\cot{x}-\cot{y}}=\frac{\cot{x}\cot{y}+1}{\cot{y}-\cot{x}}$}{51/84}
\slideq{$\sin{x+\pi}$}{52/84}
\slider{$-\sin{x}$}{52/84}
\slideq{$\lim{x\to0}{\frac{\atan{x}}{x}}$}{53/84}
\slider{$1$}{53/84}
\slideq{$\lim{x\to0}{\frac{\th{x}}{x}}$}{54/84}
\slider{$1$}{54/84}
\slideq{$\tan{x-y}$}{55/84}
\slider{$\frac{\tan{x}-\tan{y}}{1+\tan{x}\tan{y}}$}{55/84}
...
\slideq{Inégalités classiques de $\sin{x}$}{59/84}
\slider{$\forall x\in\R_+,\;\sin{x}\leqslant x$\linebreak$\forall x\in\R_-,\;\sin{x}\geqslant x$\linebreak$\forall x\in\R,\;\left|\sin{x}\right|\leqslant\left|x\right|$\linebreak$\forall x\in\left[0,\oldfrac{\pi}{2}\right],\;\sin{x}\geqslant\oldfrac{2x}{\pi}$}{59/84}
...
\slider{$\sin{x}\cos{y}$}{63/84}
\slideq{$\cos{x}+\cos{y}$}{64/84}
\slider{$2\cos{\frac{x+y}{2}}\cos{\frac{x-y}{2}}$}{64/84}
\slideq{$\cot{x}$\linebreak\linebreak$t=\tan{\frac{x}{2}}$}{65/84}
\slider{$\frac{1-t^2}{2t}$\linebreak\linebreak$t=\tan{\frac{x}{2}}$}{65/84}
\slideq{Argument d'un nombre complexe $z$ avec $\atan{x}$}{66/84}
\slider{$\arg\l z\r\equiv\atan{\frac{\Im\l z\r}{\Re\l z\r}}\,\left[\pi\right]$}{66/84}
