...
\slider{$\cos{2x}$}{1/84}
\slideq{$\sin{x}+\sin{y}$}{2/84}
\slider{$2\sin{\frac{x+y}{2}}\cos{\frac{x-y}{2}}$}{2/84}
\slideq{$\cos{x}+\cos{y}$}{3/84}
\slider{$2\cos{\frac{x+y}{2}}\cos{\frac{x-y}{2}}$}{3/84}
\slideq{$\sin{x}\cos{y}$}{4/84}
\slider{$\frac{1}{2}\l\sin{x+y}+\sin{x-y}\r$}{4/84}
\slideq{$\lim{x\to0}{\frac{\atan{x}}{x}}$}{5/84}
\slider{$1$}{5/84}
...
\slideq{$\cos{x}\cos{y}+\sin{x}\sin{y}$}{18/84}
\slider{$\cos{x-y}$}{18/84}
...
\slideq{$\frac{2t}{1+t^2}$\linebreak\linebreak$t=\tan{\frac{x}{2}}$}{22/84}
\slider{$\sin{x}$\linebreak\linebreak$t=\tan{\frac{x}{2}}$}{22/84}
\slideq{Argument d'un nombre complexe $z$ avec $\atan{x}$}{23/84}
\slider{$\arg\l z\r\equiv\atan{\frac{\Im\l z\r}{\Re\l z\r}}\,\left[\pi\right]$}{23/84}
...
\slideq{$\der{\acos{x}}$}{25/84}
\slider{$-\frac{1}{\sqrt{1-x^2}}$}{25/84}
\slideq{$n\in\N^*$, $x\in\R$\linebreak$\der[n]{\cos{x}}$}{26/84}
\slider{$\cos{x+\frac{n\pi}{2}}=\left\{\begin{matrix*}[l]\l-1\r^{\left\lfloor\oldfrac{n}{2}\right\rfloor}\cos{x}&\;\text{si }n\in2\N\\\l-1\r^{\left\lfloor\oldfrac{n}{2}\right\rfloor+1}\sin{x}&\;\text{sinon}\end{matrix*}\right.\!$}{26/84}
\slideq{$\lim{x\to0}{\frac{\tan{x}}{x}}$}{27/84}
\slider{$1$}{27/84}
\slideq{$\cos{x}\cos{y}-\sin{x}\sin{y}$}{28/84}
\slider{$\cos{x+y}$}{28/84}
\slideq{$\der{\ath{x}}$}{29/84}
\slider{$\frac{1}{1-x^2}$}{29/84}
\slideq{$-2\sin{\frac{x+y}{2}}\sin{\frac{x-y}{2}}$}{30/84}
\slider{$\cos{x}-\cos{y}$}{30/84}
\slideq{$\cot{x}$\linebreak\linebreak$t=\tan{\frac{x}{2}}$}{31/84}
\slider{$\frac{1-t^2}{2t}$\linebreak\linebreak$t=\tan{\frac{x}{2}}$}{31/84}
...
\slideq{$\lim{x\to0}{\frac{1-\cos{x}}{x^2}}$}{33/84}
\slider{$\frac{1}{2}$}{33/84}
\slideq{$\cos{\frac{\pi}{2}-x}$}{34/84}
\slider{$\sin{x}$}{34/84}
\slideq{Courbe représentative de $\asin{x}$}{35/84}
\slider{\begin{tikzpicture} \draw[color=gray,opacity=0.2] (-1.2,-2.2) grid[ystep=pi/2](1.2,2.2); \draw[->] (-1.2, 0) -- (1.2, 0) node[above left]{$x$}; \draw[->] (0,-2.2) -- (0, 2.2) node[below right]{$y$}; \foreach \y[count=\cnt] in {-\frac{\pi}{2}} \draw(0,{(-\cnt/2)*pi}) node[left]{\footnotesize$\y$}; \foreach \y[count=\cnt] in {\frac{\pi}{2}} \draw(0,{(\cnt/2)*pi}) node[left]{\footnotesize$\y$}; \foreach \x in {-1} \draw(\x,0) node[below]{\footnotesize$\x$}; \foreach \x in {1} \draw(\x,0) node[below]{\footnotesize$\x$}; \draw(0,0) node[below left]{\footnotesize$0$}; \draw[domain=-pi/2:pi/2, samples=400, scale=1, smooth, variable=\x, black] plot ({sin(\x r)},\x); \node[above] at (current bounding box.north) {$y=\asin{x}$}; \end{tikzpicture}}{35/84}
\slideq{Propriété remarquable de $\acos{x}$ et $\asin{x}$}{36/84}
\slider{$\acos{x}+\asin{x}=\frac{\pi}{2}$}{36/84}
\slideq{$\sin{x+\frac{\pi}{2}}$}{37/84}
\slider{$\cos{x}$}{37/84}
\slideq{$\cot{x+y}$}{38/84}
\slider{$\frac{\cot{x}\cot{y}-1}{\cot{x}+\cot{y}}$}{38/84}
\slideq{$\lim{x\to0}{\frac{\th{x}}{x}}$}{39/84}
\slider{$1$}{39/84}
\slideq{$\sin{x+y}$}{40/84}
\slider{$\sin{x}\cos{y}+\sin{y}\cos{x}$}{40/84}
\slideq{$\cot{x-y}$}{41/84}
\slider{$\frac{-\cot{x}\cot{y}-1}{\cot{x}-\cot{y}}=\frac{\cot{x}\cot{y}+1}{\cot{y}-\cot{x}}$}{41/84}
\slideq{$\der{\th{x}}$}{42/84}
\slider{$1-\th[2]{x}=\frac{1}{\ch[2]{x}}$}{42/84}
\slideq{Propriétés remarquables de $\atan{x}$}{43/84}
\slider{$\forall x\in\R,\;\tan{\atan{x}}=x$\linebreak$\forall x\in\left]-\oldfrac{\pi}{2},\oldfrac{\pi}{2}\right[,\;\atan{\tan{x}}=x$\linebreak$\forall n\in\N,\;\forall x\in\left]-\oldfrac{\pi}{2}+n\pi,\oldfrac{\pi}{2}+n\pi\right[$\linebreak$\atan{\tan{x}}=x-n\pi$\linebreak$\forall x\in\R^*,\;\atan{x}+\atan{\oldfrac{1}{x}}=\oldfrac{x}{\left|x\right|}\oldfrac{\pi}{2}$}{43/84}
\slideq{Inégalités classiques de $\tan{x}$}{44/84}
\slider{$\forall x\in\left[0,\oldfrac{\pi}{2}\right[,\;\tan{x}\leqslant x$\linebreak$\forall x\in\left]\oldfrac{\pi}{2},0\right],\;\tan{x}\geqslant x$\linebreak$\forall x\in\left]-\oldfrac{\pi}{2},\oldfrac{\pi}{2}\right[,\;\left|\tan{x}\right|\geqslant\left|x\right|$}{44/84}
...
\slider{$\cos{x}$}{47/84}
\slideq{Inégalités classiques de $\sin{x}$}{48/84}
\slider{$\forall x\in\R_+,\;\sin{x}\leqslant x$\linebreak$\forall x\in\R_-,\;\sin{x}\geqslant x$\linebreak$\forall x\in\R,\;\left|\sin{x}\right|\leqslant\left|x\right|$\linebreak$\forall x\in\left[0,\oldfrac{\pi}{2}\right],\;\sin{x}\geqslant\oldfrac{2x}{\pi}$}{48/84}
...
\slideq{$n\in\N^*$, $x\in\R$\linebreak$\der[n]{\ch{x}}$}{50/84}
\slider{$\left\{\begin{matrix*}[l]\ch{x}&\;\text{si }n\in2\N\\\sh{x}&\;\text{sinon}\end{matrix*}\right.\!$}{50/84}
\slideq{$\der{\ach{x}}$}{51/84}
\slider{$\frac{1}{\sqrt{x^2-1}}$}{51/84}
\slideq{$\cos{x}\sin{y}$}{52/84}
\slider{$\frac{1}{2}\l\sin{x+y}-\sin{x-y}\r$}{52/84}
\slideq{$\lim{x\to0}{\frac{\sin{x}}{x}}$}{53/84}
\slider{1}{53/84}
\slideq{$\tan{2x}$}{54/84}
\slider{$\frac{2\tan{x}}{1-\tan[2]{x}}$}{54/84}
\slideq{$\frac{\cot[2]{x}-1}{2\cot{x}}$}{55/84}
\slider{$\cot{2x}$}{55/84}
\slideq{$\lim{x\to0}{\frac{\th{x}}{x}}$}{56/84}
\slider{$1$}{56/84}
\slideq{$2\sin{x}\cos{x}$}{57/84}
\slider{$\sin{2x}$}{57/84}
...
\slideq{$\frac{\tan{x}-\tan{y}}{1+\tan{x}\tan{y}}$}{65/84}
\slider{$\tan{x-y}$}{65/84}
\slideq{$\cos{x+\pi}$}{66/84}
\slider{$-\cos{x}$}{66/84}
...
\slideq{$\der{\asin{x}}$}{70/84}
\slider{$\frac{1}{\sqrt{1-x^2}}$}{70/84}
\slideq{$\sin{x+\pi}$}{71/84}
\slider{$-\sin{x}$}{71/84}
\slideq{$\cos{x+\frac{\pi}{2}}$}{72/84}
\slider{$-\sin{x}$}{72/84}
\slideq{$\cos{\pi-x}$}{73/84}
\slider{$-\cos{x}$}{73/84}
\slideq{$n\in\N^*$, $x\in\R$\linebreak$\der[n]{\sh{x}}$}{74/84}
\slider{$\left\{\begin{matrix*}[l]\sh{x}&\;\text{si }n\in2\N\\\ch{x}&\;\text{sinon}\end{matrix*}\right.\!$}{74/84}
...
\slideq{Expression de $\ath{x}$}{77/84}
\slider{$\frac{1}{2}\ln{\frac{1+x}{1-x}}=\ln{\sqrt{\frac{1+x}{1-x}}}$}{77/84}
\slideq{$\der{\atan{x}}$}{78/84}
\slider{$\frac{1}{1+x^2}$}{78/84}
\slideq{$\sin{x}-\sin{y}$}{79/84}
\slider{$2\cos{\frac{x+y}{2}}\sin{\frac{x-y}{2}}$}{79/84}
\slideq{$\sin[2]{x}$}{80/84}
\slider{$\frac{1-\cos{2x}}{2}$}{80/84}
\slideq{$\frac{1}{2}\l\cos{x-y}-\cos{x+y}\r$}{81/84}
\slider{$\sin{x}\sin{y}$}{81/84}
\slideq{Valeurs remarquables de $\atan{x}$}{82/84}
...
\slideq{Valeurs remarquables}{83/84}
...
\slideq{$\sin{x-y}$}{84/84}
\slider{$\sin{x}\cos{y}-\sin{y}\cos{x}$}{84/84}
\end{document}