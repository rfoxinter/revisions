...
\slideq{Valeurs remarquables}{12/84}
...
\slider{$1$}{23/84}
\slideq{$\cos{x+\frac{\pi}{2}}$}{24/84}
\slider{$-\sin{x}$}{24/84}
\slideq{$n\in\N^*$, $x\in\R$\linebreak$\der[n]{\cos{x}}$}{25/84}
\slider{$\cos{x+\frac{n\pi}{2}}=\left\{\begin{matrix*}[l]\l-1\r^{\left\lfloor\oldfrac{n}{2}\right\rfloor}\cos{x}&\;\text{si }n\in2\N\\\l-1\r^{\left\lfloor\oldfrac{n}{2}\right\rfloor+1}\sin{x}&\;\text{sinon}\end{matrix*}\right.\!$}{25/84}
\slideq{$\sin{x+\frac{\pi}{2}}$}{26/84}
\slider{$\cos{x}$}{26/84}
\slideq{$\cos{x}$\linebreak\linebreak$t=\tan{\frac{x}{2}}$}{27/84}
\slider{$\frac{1-t^2}{1+t^2}$\linebreak\linebreak$t=\tan{\frac{x}{2}}$}{27/84}
\slideq{$\cos{x}\cos{y}$}{28/84}
\slider{$\frac{1}{2}\l\cos{x+y}+\cos{x-y}\r$}{28/84}
\slideq{$\sin{\pi-x}$}{29/84}
\slider{$\sin{x}$}{29/84}
\slideq{$\lim{x\to0}{\frac{\th{x}}{x}}$}{30/84}
\slider{$1$}{30/84}
\slideq{Courbe représentative de $\th{x}$}{31/84}
\slider{\begin{tikzpicture} \draw[color=gray,opacity=0.2] (-5.2,-1.2) grid (5.2,1.2); \draw[->] (-5.2, 0) -- (5.2, 0) node[above left]{$x$}; \draw[->] (0,-1.2) -- (0, 1.2) node[below right]{$y$}; \foreach \y in {-1,1} \draw(0,\y) node[left]{\footnotesize$\y$}; \foreach \x in {-5,...,-1} \draw(\x,0) node[below]{\footnotesize$\x$}; \foreach \x in {1,...,5} \draw(\x,0) node[below]{\footnotesize$\x$}; \draw(0,0) node[below left]{\footnotesize$0$}; \draw[domain=-5.2:5.2, samples=1100, scale=1, smooth, variable=\x, black] plot (\x,{(exp(\x)-exp(-\x))/(exp(\x)+exp(-\x))}); \node[above] at (current bounding box.north) {$y=\th{x}$}; \end{tikzpicture}}{31/84}
\slideq{Expression de $\ath{x}$}{32/84}
\slider{$\frac{1}{2}\ln{\frac{1+x}{1-x}}=\ln{\sqrt{\frac{1+x}{1-x}}}$}{32/84}
\slideq{Valeurs remarquables de $\asin{x}$ et $\acos{x}$}{33/84}
...
\slideq{$\lim{x\to0}{\frac{1-\cos{x}}{x^2}}$}{34/84}
\slider{$\frac{1}{2}$}{34/84}
\slideq{$\cos{x}\sin{y}$}{35/84}
\slider{$\frac{1}{2}\l\sin{x+y}-\sin{x-y}\r$}{35/84}
\slideq{$\lim{x\to0}{\frac{\ch{x}-1}{x^2}}$}{36/84}
\slider{$\frac{1}{2}$}{36/84}
\slideq{Valeurs remarquables de $\atan{x}$}{37/84}
...
\slideq{Propriété remarquable de $\acos{x}$ et $\asin{x}$}{51/84}
\slider{$\acos{x}+\asin{x}=\frac{\pi}{2}$}{51/84}
\slideq{$\lim{x\to0}{\frac{\th{x}}{x}}$}{52/84}
\slider{$1$}{52/84}
...
\slideq{$\der{\acos{x}}$}{54/84}
\slider{$-\frac{1}{\sqrt{1-x^2}}$}{54/84}
\slideq{$\cos{x+\pi}$}{55/84}
\slider{$-\cos{x}$}{55/84}
\slideq{$\der{\asin{x}}$}{56/84}
\slider{$\frac{1}{\sqrt{1-x^2}}$}{56/84}
\slideq{$\frac{1}{2}\l\sin{x+y}+\sin{x-y}\r$}{57/84}
\slider{$\sin{x}\cos{y}$}{57/84}
\slideq{Inégalités classiques de $\tan{x}$}{58/84}
\slider{$\forall x\in\left[0,\oldfrac{\pi}{2}\right[,\;\tan{x}\leqslant x$\linebreak$\forall x\in\left]\oldfrac{\pi}{2},0\right],\;\tan{x}\geqslant x$\linebreak$\forall x\in\left]-\oldfrac{\pi}{2},\oldfrac{\pi}{2}\right[,\;\left|\tan{x}\right|\geqslant\left|x\right|$}{58/84}
\slideq{Courbe représentative de $\ch{x}$}{59/84}
\slider{\begin{tikzpicture} \draw[color=gray,opacity=0.2] (-2.7,-0.2) grid (2.7,5.2); \draw[->] (-2.7, 0) -- (2.7, 0) node[above left]{$x$}; \draw[->] (0,-0.7) -- (0, 5.2) node[below right]{$y$}; \foreach \y in {1,...,5} \draw(0,\y) node[left]{\footnotesize$\y$}; \foreach \x in {-2,-1} \draw(\x,0) node[below]{\footnotesize$\x$}; \foreach \x in {1,2} \draw(\x,0) node[below]{\footnotesize$\x$}; \draw(0,0) node[below left]{\footnotesize$0$}; \draw[domain=-2.333:2.333, samples=500, scale=1, smooth, variable=\x, black] plot (\x,{(exp(\x)+exp(-\x))/2}); \node[above] at (current bounding box.north) {$y=\ch{x}$}; \end{tikzpicture}}{59/84}
\slideq{$\frac{\tan{x}-\tan{y}}{1+\tan{x}\tan{y}}$}{60/84}
\slider{$\tan{x-y}$}{60/84}
...
\slideq{Expression de $\ach{x}$}{62/84}
\slider{$\ln{x+\sqrt{x^2-1}}$}{62/84}
\slideq{$\cos{\pi-x}$}{63/84}
\slider{$-\cos{x}$}{63/84}
\slideq{$\der{\ath{x}}$}{64/84}
\slider{$\frac{1}{1-x^2}$}{64/84}
\slideq{$\frac{-\cot{x}\cot{y}-1}{\cot{x}-\cot{y}}=\frac{\cot{x}\cot{y}+1}{\cot{y}-\cot{x}}$}{65/84}
\slider{$\cot{x-y}$}{65/84}
\slideq{$n\in\N^*$, $x\in\R$\linebreak$\der[n]{\sh{x}}$}{66/84}
\slider{$\left\{\begin{matrix*}[l]\sh{x}&\;\text{si }n\in2\N\\\ch{x}&\;\text{sinon}\end{matrix*}\right.\!$}{66/84}
\slideq{Inégalités classiques de $\sh{x}$}{67/84}
\slider{$\forall x\in\R_+,\;\sh{x}\geqslant x$\linebreak$\forall x\in\R_-,\;\sh{x}\leqslant x$}{67/84}
\slideq{$n\in\N^*$, $x\in\R$\linebreak$\der[n]{\sin{x}}$}{68/84}
\slider{$\sin{x+\frac{n\pi}{2}}=\left\{\begin{matrix*}[l]\l-1\r^{\left\lfloor\oldfrac{n}{2}\right\rfloor}\sin{x}&\;\text{si }n\in2\N\\\l-1\r^{\left\lfloor\oldfrac{n}{2}\right\rfloor}\cos{x}&\;\text{sinon}\end{matrix*}\right.\!$}{68/84}
\slideq{$\frac{\cot[2]{x}-1}{2\cot{x}}$}{69/84}
\slider{$\cot{2x}$}{69/84}
\slideq{Courbe représentative de $\acos{x}$}{70/84}
\slider{\begin{tikzpicture} \draw[color=gray,opacity=0.2] (-1.2,-0.2) grid[ystep=pi/2](1.2,3.2); \draw[->] (-1.2, 0) -- (1.2, 0) node[above left]{$x$}; \draw[->] (0,-0.2) -- (0, 3.2) node[below right]{$y$}; \foreach \y[count=\cnt] in {\frac{\pi}{2},\pi} \draw(0,{(\cnt/2)*pi}) node[left]{\footnotesize$\y$}; \foreach \x in {-1} \draw(\x,0) node[below]{\footnotesize$\x$}; \foreach \x in {1} \draw(\x,0) node[below]{\footnotesize$\x$}; \draw(0,0) node[below left]{\footnotesize$0$}; \draw[domain=pi:0, samples=400, scale=1, smooth, variable=\x, black] plot ({cos(\x r)},\x); \node[above] at (current bounding box.north) {$y=\acos{x}$}; \end{tikzpicture}}{70/84}
\slideq{$\cos[2]{x}-\sin[2]{x}=2\cos[2]{x}-1=1-2\sin[2]{x}$}{71/84}
\slider{$\cos{2x}$}{71/84}
\slideq{$\sin{x}\cos{y}-\sin{y}\cos{x}$}{72/84}
\slider{$\sin{x-y}$}{72/84}
\slideq{$\sin{\frac{\pi}{2}-x}$}{73/84}
\slider{$\cos{x}$}{73/84}
\slideq{$\frac{1}{2}\l\cos{x-y}-\cos{x+y}\r$}{74/84}
\slider{$\sin{x}\sin{y}$}{74/84}
\slideq{Expression de $\ash{x}$}{75/84}
\slider{$\ln{x+\sqrt{x^2+1}}$}{75/84}
\slideq{$\der{\ash{x}}$}{76/84}
\slider{$\frac{1}{\sqrt{x^2+1}}$}{76/84}
\slideq{$\lim{x\to0}{\frac{\asin{x}}{x}}$}{77/84}
\slider{$1$}{77/84}
...
\slider{$\tan{x}$\linebreak\linebreak$t=\tan{\frac{x}{2}}$}{80/84}
\slideq{$\sin{x+y}$}{81/84}
\slider{$\sin{x}\cos{y}+\sin{y}\cos{x}$}{81/84}
\slideq{$\cos{x}\cos{y}-\sin{x}\sin{y}$}{82/84}
\slider{$\cos{x+y}$}{82/84}
